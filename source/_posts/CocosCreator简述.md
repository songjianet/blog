...
- <font color=#fc6423>在对文件或文件夹执行创建、修改和删除操作的时候建议在`CocosCreator`编辑器中执行，通过`CocosCreator`编辑器之外对文件或文件夹的任何操作都无法记录到`meta`文件。</font>
...
- <font color=#fc6423>`canvas`会根据不同的屏幕自动进行一定的适配，但是这并不是最完全的解决方案。</font>
...
- <font color=#fc6423>浏览器预览不会自动播放背景音乐，需要进行一次点击才会播放。</font>
