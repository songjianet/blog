...
- 对于之前《自定义`Palette`》、《自定义`Render`》和《自定义`ContextPad`》三个部分的文章而言，在`Palette`中添加一个节点就需要在自定义的`Palette`中加入该节点的方法并且还需要将其`return`出去。

- 当存在多个节点或者通过后台进行管理节点的时候，以现有的方式开发者还需要对其进行大量的代码修改。

- 想要解决这个问题，可以使用面向对象中的封装思想，通过一个`json`格式的配置文件，对里面的所有组件进行一定的操作，同时需要编写一个方法用于自定根据`json`配置文件去生成节点。
